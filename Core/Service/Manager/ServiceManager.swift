...
class ServiceManager: FirebaseService {
    ...
    func update(userId: String, collectionPath: String, docId: String, data: [String : Any]) async throws {
        try await FirebaseConstants.collectionPath(userId: userId, collectionId: collectionPath).document(docId).setData(data, merge: true)
    }
